...
This report demonstrates the analysis of survey data collected using the **shinysurveys** package. The goal is to explore the data and provide students with a practical introduction to creating dynamic reports using Quarto.
...
```{r poisson-model-real, eval=FALSE}
# Example Poisson regression with real data
# NOT RUN
library(see)
library(parameters)
 
poisson_model <- glm(tvhours ~ age + race + rincome + partyid, family = poisson, data = tvhours_data)
...
This Quarto report provides an introduction to survey data analysis, including descriptive statistics and visualizations. By using Quarto, we can create dynamic and reproducible reports suitable for teaching and research purposes.

Lorem ipsum dolor sit amet, consectetur adipiscing elit. Proin hendrerit ipsum nec purus pharetra, ac varius augue posuere.
